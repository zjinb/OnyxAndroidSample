...
//        final TextView dateDisplay = (TextView) clock.findViewById(R.id.date);
        final TextTime dateDisplay = (TextTime) clock.findViewById(R.id.digital_clock);
        if (dateDisplay == null) {
            return;
        }
...
//    }
     ...
     * @param textTime
     */
    public static void setTimeFormat(Context context, TextTime textTime){
        if (textTime != null){
            textTime.setFormat12Hour(get12ModeFormat(context, true));
            textTime.setFormat24Hour(get24ModeFormat());
        }
    }
    ...
    public static CharSequence get12ModeFormat(Context context, boolean showAmPm) {
        String pattern = "";
        if (Build.VERSION.SDK_INT >= 18){
            pattern = DateFormat.getBestDateTimePattern(Locale.getDefault(), "hma");
        } else {
            java.text.DateFormat timeFormat =
                    java.text.DateFormat.getTimeInstance(java.text.DateFormat.SHORT,
                            Locale.getDefault());
...
//    }
    ...
    public static <E> HashSet<E> newHashSet(Collection<E> collection) {
        final HashSet<E> hashSet = new HashSet<>(collection.size());
