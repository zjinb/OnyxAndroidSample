...
public abstract class GridItemBaseAdapter extends OnyxPagedAdapter
{
    ...
    private boolean mIsInMultipleSelectionMode = false;
    ...
    public boolean removeItem(GridItemData item)
    {
        if (mItems.remove(item)) {
            this.getPaginator().setItemCount(mItems.size());
            if (this.getPaginator().getPageIndex() >= this.getPaginator().getPageCount()) {
                this.getPaginator().setPageIndex(this.getPaginator().getPageCount() - 1);
            }
            return true;
        }
...
//        }
    ...
    {
        return mIsInMultipleSelectionMode;
    }

    public void setMultipleSelectionMode(boolean value)
    {
        if (mIsInMultipleSelectionMode != value) {
            mIsInMultipleSelectionMode = value;
            this.notifyDataSetChanged();
        }
    }
